...
```{r pp}
paintings <- read_csv("data/paris-paintings.csv") %>% 
  clean_names() %>% 
...
```{r gg-height-width}
measurements_scatter <- paintings %>% 
  ggplot(aes(x = width_in, y = height_in)) +
...
$$\large{\hat{\text{height_in}}} = 3.62 + 0.78\text{width_in}$$
...
and the fitted value. The residuals and fitted values can be extracted using `augment()` function from `broom`:
...
mod_heights_sl <- lm(height_in ~ ---, data = paintings)
tidy(---)
```
